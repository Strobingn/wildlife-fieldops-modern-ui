 ...
 * Skips properties that are already frozen for speed.
 *
 * @template T
 * @param {T} obj
 * @returns {T}
...
export function createStore(initialState) {
  ...
  return {
    /**
     * Get current state snapshot (deep-frozen for immutability).
     * Avoids O(N) deepClone overhead on state reads while preserving safety.
     * @returns {T}
     */
    getState() {
      return state;
     ...
     * Deep-freezes next state to ensure strict immutability without O(N) deepClone overhead.
