...
export function createStore(initialState) {
  ...
  // Freeze top-level state to enforce immutability without O(N) deep clones
  ...
  return {
     ...
     * O(1) performance instead of O(N) deepClone on every read.
     * @returns {T}
     */
    getState() {
      return state;
    ...
    setState(updater) {
      ...
      // Notify subscribers with frozen state snapshot (bypasses redundant O(N) deepClone)
      if (!isNotifying) {
        isNotifying = true;
