...
export function createStore(initialState) {
  ...
  // Freeze state to ensure immutability without expensive deep cloning
  ...
  return {
    /**
     * Get current state reference (O(1) immutable read instead of O(N) deepClone serialization).
     * @returns {T}
     */
    getState() {
      return state;
    ...
    setState(updater) {
      ...
      // Notify subscribers with frozen state snapshot (avoiding O(N) deepClone on every notification)
      if (!isNotifying) {
        isNotifying = true;
