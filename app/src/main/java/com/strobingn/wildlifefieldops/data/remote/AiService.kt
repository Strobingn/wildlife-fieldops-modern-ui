...
/**
 * TEMPORARY compile-safe stub. Full AiService must be restored from box path
 * /workspace/wildlife-fieldops-modern-ui/.../AiService.kt (or /tmp/c.txt).
 */
@Singleton
class AiService @Inject constructor(
    ...
    suspend fun ask(userMessage: String, species: String = ""): String = withContext(Dispatchers.IO) {
        JobIntakeParser.parse(
            ...
            completeCloud = { _, _ -> null to "stub" },
            providerLabel = providerLabel,
            localDisplayName = modelManager.activeDisplayName,
            notConfiguredMessage = "AiService stub — restore full file"
        ).error ?: "AiService stub active. Restore full AiService.kt from agent box."
    }
    ...
    suspend fun draftEstimateFromJob(
        ...
        taxRate = taxPercent,
        rationale = "AiService stub — restore full file. $distanceNote"
    )
